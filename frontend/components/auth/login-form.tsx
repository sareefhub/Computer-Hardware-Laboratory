...
function deriveRole(u: string): UserRole {
  const v = u.trim().toLowerCase()
  if (v.startsWith("admin")) return "admin"
  if (v.startsWith("teacher")) return "teacher"
  return "student"
}
    ...
    <div className="flex min-h-[70vh] items-center justify-center p-4">
      <Card className="w-full max-w-md border-slate-200/60 shadow-lg">
        <div className="flex flex-col items-center gap-2 pt-8">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#1E4B9B] text-white text-lg font-bold shadow-sm">PSU</div>
          <h2 className="text-xl font-semibold text-slate-900">เข้าสู่ระบบ</h2>
          <p className="text-sm text-slate-500">กรอกชื่อผู้ใช้และรหัสผ่านเพื่อใช้งานระบบ</p>
        </div>
        <CardContent className="space-y-5 pt-6 pb-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                id="username"
                type="text"
                placeholder="เช่น student123 หรือ 6510110115"
                value={username}
          ...
          <div className="pt-2 text-center text-sm text-slate-500 space-y-1">
            ...
            <div>นักศึกษา: student123 / password</div>
            <div>อาจารย์: teacher123 / password</div>
            <div>ผู้ดูแลระบบ: admin123 / password</div>
          </div>
        </CardContent>
      </Card>
    </div>
